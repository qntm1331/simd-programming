...
int main() {
    // 변수 선언
    int i, x, total, nthreads, constant, sum[4][PAD];
    constant = 12;
    ...
    #pragma omp parallel
    {
        ...
        //     스레드 2 → 2, 6, 10, 14, ...
        //     스레드 3 → 3, 7, 11, 15, ...
        for (i = uniqueID, sum[uniqueID][0] = 0.0;
             i < num_steps;
             i += nthrds)
        {
            x = i + constant;
            // 각 스레드는 자신의 sum 배열에만 결과 저장
            // sum[uniqueID][0]만 사용, 나머지 [1]~[7]은 패딩
            sum[uniqueID][0] += x;
        }
